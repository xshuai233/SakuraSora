...
class Ui_Form(object):
    def setupUi(self, Form):
        ...
        self.model.setHorizontalHeaderLabels(['订单编号', '物品名称', '物品总重(kg)', '物品发送时间', '发送方名称', '订单状态'])
        self.tableView.setModel(self.model)
        self.tableView.horizontalHeader().setSectionResizeMode(QHeaderView.Stretch)
        self.pushButton_2 = QtWidgets.QPushButton(Form)
        self.pushButton_2.setGeometry(QtCore.QRect(660, 20, 121, 28))
    ...
    def removeOrder(self):
        ...
        for i in rows:
            ...
            if flag == 0:
                cu.execute("UPDATE order_ SET isCanceled = 2 WHERE ID = '{t0}';".format(t0 = ID))
                co.commit()
            elif flag == 1: 
